...
async def generate_ai_analysis(score: int, rank: int, province: str, category: str, subject: str, majors: list) -> str:
    ...
    prompt = f"""你是一位有20年经验的高考志愿规划师，正在帮助一个四川{category}考生做志愿决策。这个孩子考了{score}分（全省位次{rank}名），选科是{subject}。
...
请你认真分析，给出真正负责任的建议。这个孩子和家长会完全相信你的话，你的每一句话都可能影响他的人生轨迹。

请用300字以内回答以下4个问题（说人话，不要套话）：

1.【首选推荐】这些方向你最推荐哪2-3个？为什么？（考虑：就业确定性、考公优势、薪资天花板、35岁风险）
2.【避坑提醒】有没有看似还行但实际要小心的？（比如：学校看起来好但专业方向偏、中外合作溢价高、行业萎缩风险）
3.【考公路线】如果孩子想考公务员，哪些专业有优势？具体可以报什么岗位？
4.【一句话总结】给家长和孩子一句话建议。
